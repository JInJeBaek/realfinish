...
Bot.choseMenu = function(req, content, callback){

    switch (content) {
        case message.buttons[0]: //교내식단
            RedisDAO.getByKey(req.cache, RedisDAO.key_diet_normal, function(err, result){
                callback(err, message.baseType(JSON.parse(result)));
    });
    break;
case message.buttons[1]: //BTL식단
    RedisDAO.getByKey(req.cache, RedisDAO.key_diet_BTL, function(err, result){
        callback(err, message.baseType(JSON.parse(result)));
});
    break;
